...
function jasshelper.do_compile(self, map_path, common_j_path, blizzard_j_path, option)
	local parameter = ""
	if tonumber(xjass.config.cJass.Enable) == 1 then
        require "compile.cjass"
        cjass : compile(map_path, option)
	end
	...
	if option.enable_jasshelper_scriptonly then
		...
		local result = sys.spawn(command_line, fs.ydwe_path(), true, true)
        ...
        if mpq then
            ...
            if mpq:extract("war3map.j", out_file) then
                ...
                ----------------------------
                local fkdn = tonumber(xjass.config.xJass.fkDoNothing)
                if fkdn ~= 3 then
                    package.loaded['compile.xjass.XGfkDoNothing'] = nil
                    require(  'compile.xjass.XGfkDoNothing' )
                    jass = XG_fkDoNothing_Do(jass, fkdn)
                end
                ------------------------
                if tonumber(xjass.config.xJass.S2Hopt) == 1 then
                    log.trace("XG S2Hopt starting")
                    package.loaded['compile.xjass.XGS2Hopt'] = nil
                    require(  'compile.xjass.XGS2Hopt' )
                    jass = XG_S2H_Compile(jass)
                end
				...
				if tonumber(xjass.def['xjass']['XGHandleHelperOn']) == 1 then
					log.trace("XGHandleHelper forceinclude")
					package.loaded['compile.xjass.XGHandleHelper'] = nil
                    jass = require(  'compile.xjass.XGHandleHelper' ):Init(jass)
				end
