...
-- 启动xJass配置对话框
local function launch_xjassconfig()
	local config_program = fs.ydwe_path() / "plugin" / "HCcompiler" / "HCcompiler.exe"
	local command_line = string.format('"%s"', config_program:string())
	sys.spawn(command_line, fs.ydwe_path())
end

-- 使用11测试
local function Test_On_11()
...
local function open_feedback_site()
    sys.spawn('explorer "https://create.reckfeng.com/feedback/"')
end
...
function event.EVENT_INIT_MENU(event_data)
	...
	menu:add(_("&XJass Config"), launch_xjassconfig)
