...
local function compile_map(map_path, option)
    ...
    if option.enable_jasshelper then
        ...
        if result then
            ...
            log.trace("#xjass#")
            require "compile.xjass"
            xjass : compile
            (
                map_path, --map_path[map]
                '', --temp_path[mix]: xxx\xxx.w3xTemp
                '', --jass_path[mix]: xxx\xxx.w3xTemp\war3map.j
                compile_t.input:parent_path() / (compile_t.input:stem():string() .. ".xj") --output_path[mix]
            )
