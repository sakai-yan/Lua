...
private function DamAction_Fix takes nothing returns nothing
	...
	local real dam = LoadReal(ExecuteQue, id, JAPIDAM_DAM)
	local real Odam = LoadReal(ExecuteQue, id, JAPIDAM_Odam)
	...
	if dam > Odam then //--补足[伤害差>0]--
		if life - dam > 0.405 then //非致死伤害
			call SetUnitState(ue, UNIT_STATE_LIFE, life - (dam - Odam) + 0.001)
...
private function DamAction_Redirect takes nothing returns boolean //自带伤害 - 重定向
	...
	local weapontype wp = null
	local damagetype dp = null
	local boolean atk = YDWEIsEventAttackDamage()
	set ExecuteQueId[0] = ExecuteQueId[0] + 1
	call SaveBoolean( ExecuteQue, ExecuteQueId[0], JAPIDAM_ATTACK, atk )
	if atk then //技能伤害获取远程和武器类型必蹦
		set rng = YDWEIsEventRangedDamage()
		set wp = ConvertWeaponType(EXGetEventDamageData(5))
	else //平A获取伤害类型会蹦
		set dp = ConvertDamageType(EXGetEventDamageData(4))
		//BJDebugMsg("ffff")
	endif
	...
	call Damage_Deal_Code(us, ue, RAbsBJ(dam), false, YDWEIsEventPhysicalDamage(), rng, ConvertAttackType(EXGetEventDamageData(6)), dp, wp, false)
	set us = null
	set ue = null
	return true
endfunction
