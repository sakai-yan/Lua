...
globals
	...
	private attacktype ATP = ATTACK_TYPE_NORMAL
  ...
  if not(IsUnitInGroup(Charge_Current, Group)) and Charge_Current != Charge_Source  then
   if not(Switch) and EnemyFilter(Charge_Current) then
			...
			if dam>0 and HaveSource then
				#ifndef XGDamPlus_ON
				call UnitDamageTarget(Charge_DamSource, Charge_Current, dam, false, false, ATP, DTP, WTP)
				#else
				call XG_UnitDamTarPlus(Charge_DamSource, Charge_Current, dam, false, false, ATP, DTP, WTP)
				#endif
			endif
   ...
   call DestroyEffect( AddSpecialEffect(Effe, GetUnitX(Charge_Current), GetUnitY(Charge_Current)) )
	...
	function XG_Charge_SetDamageType takes attacktype atp, damagetype dtp, weapontype wtp  returns nothing
		set ATP = atp
		set DTP = dtp
		set WTP = wtp
	endfunction
