...
#define XG_Int2AbilityId(a) (a)
#define XG_Int2Upgrade(a) (a)
#define XG_Upgrade2Int(a) (a)

#define XG_StringConcat_Lv10(s1,s2,s3,s4,s5,s6,s7,s8,s9,s10) ( s1 + s2 + s3 + s4 + s5 + s6 + s7 + s8 + s9 + s10 )

#define XG_AutoAttr_SetClassA(str) <?=XG_AutoAttr_SetClass_Lua(str)?>
#define XG_AutoAttr_AddAttrA(str,attr,v,ss) <?=XG_AutoAttr_AddAttr_Lua(str, attr, v, ss, true)?> //breakWhenMatch
#define XG_AutoAttr_AddAttrExA(regexp, key, name , code, breakWhenMatch) <?=XG_AutoAttr_AddAttr_Lua(regexp, key, name, code, breakWhenMatch)?> 
...
//快速排序
#define XG_QuickSort_int(array, order, left, right) <?=XG_QuickSort_Lua([=[array]=],order,[=[int]=])?>(left, right)
#define XG_QuickSort_real(array, order, left, right) <?=XG_QuickSort_Lua([=[array]=],order,[=[real]=])?>(left, right)

//逆天扩展
#define RemoveYDTimer(t) FlushChildHashtable(YDLOC,GetHandleId(t)) YDNL call DestroyTimer(t)
#define RemoveYDTrigger(t) FlushChildHashtable(YDLOC,GetHandleId(t)) YDNL call DestroyTrigger(t)
