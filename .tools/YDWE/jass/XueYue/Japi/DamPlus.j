...
library XGDamPlus requires XGbase,HcGetDam,HcUnitAlive,HcIsUnitInvunerable,YDWEEventDamageData,userdataGC
    globals
...
//**************************************
	...
	private function DamAction takes unit us,unit ue,real dam,boolean phy,boolean trgdam,boolean rng,attacktype atp,damagetype dtp,weapontype wtp,boolean truth returns nothing
		...
		set switch =false
		call UnitDamageTarget( us,ue, 0.01, false, true, atp, DAMAGE_TYPE_UNIVERSAL, WEAPON_TYPE_WHOKNOWS ) //非远程伤害会受到冰甲影响
		//call EnableTrigger(Xg_DamPlus_Trg)
		if testd == 0. then
		   damplus_dbg call BJDebugMsg("debug[206]: 判断无敌魔免")
		  return
		endif
		...
		if phy then
            ...
            set arm = GetUnitState(ue, ConvertUnitState(0x20))
		endif
		...
		if not truth then
			...
			if phy then
				...
				damplus_dbg call BJDebugMsg("debug[263]: 伤害:"+R2S(dam)+"  护甲:"+R2S(arm))
			else
				...
				damplus_dbg call BJDebugMsg("debug[272]: 伤害:"+R2S(dam)+"  魔抗:"+R2S(mk))
		...
		if dam > 0.0 then
			...
			damplus_dbg call BJDebugMsg("debug[340]: 伤害:"+R2S(dam)+"  护盾:"+R2S(hd))
    ...
    private function UnitDamagedAction takes nothing returns nothing
        ...
        local boolean phy = LoadBoolean(ExecuteQue, id, HA_PHY)
		if ExecuteQueId[id] != 0 then
			set rng=LoadBoolean(ExecuteQue,id,HA_RNG)
		endif
        
		if UnitAlive(ue) and testd!=0.0 and not( phy==false and IsUnitType(ue, UNIT_TYPE_MAGIC_IMMUNE) )  then
		...
		if dam > 0.0 then //--补足[伤害差>0 !与! 法术伤害]--
				...
				elseif olddam < 0.0 and life - newdam > 0.405 then //非致死的法术伤害 修正伤害
                    call EXSetEventDamage ( newdam )
				elseif olddam == 0.0 and life - dam > 0.405 then //触发伤害
				  call SetUnitState(ue, UNIT_STATE_LIFE, life - dam )
				  call DisableTrigger(Xg_DamPlus_Trg)
                  call UnitDamageTarget( us,ue, 0.00, false, rng, ATTACK_TYPE_CHAOS, DAMAGE_TYPE_UNIVERSAL, weapon ) //非远程伤害会受到冰甲影响
                  call EnableTrigger(Xg_DamPlus_Trg)
