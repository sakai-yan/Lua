    ...
    local function CreateHeightMap()
        ...
        while x <= xMax do
            ...
            while y <= yMax do
                ...
                if STORE_CLIFF_DATA then
                    local level1 = GetTerrainCliffLevel(x, y) --带有无路径纹理的可破坏物会导致报错
                    --[[local suc, level1 = xpcall(
                        GetTerrainCliffLevel,
                        function (err) print(err, ('x = %d, y = %d'):format(x, y)) end,
                        x, y
                    )]]
...
--======================
    ...
    --GetLocZ = GetLocZ,
    local XGJAPI = XGJAPI
    Xfunc['XG_HeightMap_GetTerrainZ'] = function ()
        XGJAPI.real[0] = GetTerrainZ
        (
            XGJAPI.real[1] or 0.00,
            XGJAPI.real[2] or 0.00
        )
    end
    Xfunc['XG_HeightMap_GetUnitZ'] = function ()
        local u = XGJAPI.unit[1]
        if not u or u == 0 then
            XGJAPI.real[0] = 0.00
            return
        end
        XGJAPI.real[0] = GetUnitZ(u)
    end
