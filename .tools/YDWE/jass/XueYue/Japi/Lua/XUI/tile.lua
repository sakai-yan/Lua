...
local class = {
    ...
    --边框 遮罩
    image_mask_active = 'XG_JAPI\\Lua\\XUI\\ui\\blank.blp',

    image_mask_down = 'XG_JAPI\\Lua\\XUI\\ui\\blank.blp',
...
---@return xui.tile
function xui.tile:new( params, textTemplate )
    ---@class xui.tile
    ...
    japi.DzFrameSetPoint( shell.frame_text.frame, constFrame.POINT_LT, shell.frame_background.frame, constFrame.POINT_LT, 0, 0  )
    japi.DzFrameSetPoint( shell.frame_text.frame, constFrame.POINT_RB, shell.frame_background.frame, constFrame.POINT_RB, 0, 0  )
...
function class:event( eventName, callback )
    assert(type(callback)=='function', 'tile(xui):event callback类型错误')
    eventName = map_evt_tp[eventName]
    assert(eventName, 'tile(xui):event eventName==nil' )
...
class:set_property( "text", 'set', function (class, this, key, value)
    this.data[key] = value
    this.frame_text.value = value
end )
