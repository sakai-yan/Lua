...
<?local Misc_DefenseArmor     = tonumber(require('slk').misc.Misc.DefenseArmor)?>
	function XGGetUnitArmor takes unit us,unit u,integer abilid,real testdam returns real
    local real    life = GetWidgetLife(u)
    local real    test = life
    local real    redc = 0.
    local boolean enab = false
    local trigger trig = GetTriggeringTrigger()
    local real backlife = 500.
	if <?=Misc_DefenseArmor?> ==0.00 then
		return 0.00
	endif
    if u != null and life >= 0.405 then
        if GetUnitState(u, UNIT_STATE_MAX_LIFE) <= backlife then
