...
library XGDamPlus requires XGbase,HcGetDam,HcGetUnitArmor,HcUnitAlive,HcIsUnitInvunerable,userdataGC
    globals
		...
		private integer AOEId = 0
		private boolean switch =true
		private real testd =0.
...
//**************************************
    ...
    function XG_DamPlus_SetSpecSign takes string s,string g returns nothing //#设置特殊伤害标记#
		//local x
		call SaveStr( ExecuteQue,ExecuteQueId[0]+1, StringHash( s ), g )
	endfunction
    function XG_DamPlus_GetSpecSign takes string s returns string //#获取特殊伤害标记#
		//local x
		return LoadStr( ExecuteQue,ExecuteQueId[0], StringHash( s ) )
	endfunction
    function XG_DamPlus_ChgSpecSign takes string s,string g returns nothing //#修改当前伤害标记#
		//local x
		call SaveStr( ExecuteQue,ExecuteQueId[0], StringHash( s ), g )
	endfunction
	...
	private function DamAction takes unit us,unit ue,real dam,boolean phy,boolean trgdam,boolean rng,attacktype atp,damagetype dtp,weapontype wtp,boolean truth returns nothing
		...
		local real mfhd
		local real hd
		if not(UnitAlive(ue)) then
			return
		endif
		if phy==false and IsUnitType(ue, UNIT_TYPE_MAGIC_IMMUNE) then
			damplus_dbg call BJDebugMsg("debug[203]: 判断魔免")
			return
		endif
		set switch =false
		call UnitDamageTarget( us,ue, 0.01, false, true, Xg_DamPlus_GetATP(), DAMAGE_TYPE_UNIVERSAL, WEAPON_TYPE_WHOKNOWS ) //非远程伤害会受到冰甲影响
		//call EnableTrigger(Xg_DamPlus_Trg)
		if testd == 0. then
			damplus_dbg call BJDebugMsg("debug[206]: 判断无敌")
			return
		endif
		...
		if phy then
			set arm = XGGetUnitArmor( us, ue, ReduceAbil, 240 ) //中立敌对的单位对自己造成伤害时会乱跑，所以传入一个伤害来源。
		endif
		...
		if atk then //普通攻击
		 damplus_dbg call BJDebugMsg("debug[222]: 普通攻击")
			if GetRandomReal(0.01, 1.00) <= RMinBJ( LoadReal(HT,key_ue,HA_ATTR_DODGE) ,arint[1] ) then //闪避
					...
					set dam = LoadReal(ExecuteQue, id, HA_DAM)
					damplus_dbg call BJDebugMsg("debug[230]: 伤害闪避 " + R2S(dam))
					if not LoadBoolean(ExecuteQue, id, HA_PDOD) then //没有要求无视本次闪避
						damplus_dbg call BJDebugMsg("debug[231]: 伤害闪避 " + R2S(dam))
						call Xg_DamPlus_SetDam( 0.0 )
						return
					endif
			endif
			damplus_dbg call BJDebugMsg("debug[232]: 伤害 " + R2S(dam))
		endif
		...
		if not truth then
			...
			set dam = dam * ( 1 - RMinBJ(arint[6], LoadReal(HT,key_ue,HA_ATTR_BFBJS)) ) //减伤%
			set dam = RMaxBJ(dam - rjs, 0.00) //减伤
			if phy then
				set arm = arm * ( 1 - LoadReal(HT,key_us,HA_ATTR_BFBWC) ) //护甲穿透-百分比
				...
				if arm >= 0.00 then //护甲重计算伤害
					set dam = dam * ( 1 - arm * hd / (arm * hd + 1) )
				else
                    if arm < -1500 then
                        set arm = - 1500 //最大穿透护甲  数值过大导致丢失数值
                    endif
					set dam = dam * ( 2.0 - Pow(1 - hd, -arm) )
				endif
				damplus_dbg call BJDebugMsg("debug[285]: 伤害:"+R2S(dam)+"  护甲:"+R2S(arm))
			else
				...
				if mk >= 0.0 then //魔抗重计算伤害
                     <? if type(XG.damplus['mk#1'])=='nil' then 
                        sstr = 'dam * ( 1 - mk * point / ( mk * point + 1 ) )'
                     else
                       sstr = XG.damplus['mk#1']
                     end
                     sstr = sstr:gsub('point','arint[0]')
                     ?>
					  set dam = <?=sstr?>
				else
                     <? if type(XG.damplus['mk#2'])=='nil' then 
                        sstr = 'dam * ( 2 - Pow( 1 - point, -mk ) )'
                     else
                        sstr = XG.damplus['mk#2']
                     end
                     sstr = sstr:gsub('point','arint[0]')
                     ?>
                    if mk < -1500 then
                        set mk = - 1500 //最大穿透  数值过大导致丢失数值
                    endif
					set dam = <?=sstr?>
				endif
				damplus_dbg call BJDebugMsg("debug[308]: 伤害:"+R2S(dam)+"  魔抗:"+R2S(mk))
				...
				if mfhd > 0.0 and dam > 0.0 then
					...
					call Xg_DamPlus_SetState( HA_STATE_MFHD, true )
				endif
			endif
		...
		set dam = LoadReal(ExecuteQue, id, HA_DAM)
		if atk and dam>0.0 then
			set dam = dam - LoadReal(HT,key_ue,HA_ATTR_YHPF)
		endif
		
		if dam > 0.0 then
			set hd = LoadReal(HT,key_ue,HA_ATTR_HD)
			damplus_dbg call BJDebugMsg("debug[344]: 伤害:"+R2S(dam)+"  护盾:"+R2S(hd))
			if hd > 0.0 and dam > 0.0 then
					call Xg_DamPlus_SetState( HA_STATE_HD, true )
					if hd >= dam then
						set hd = hd - dam
						set dam = 0.0
					else
	...
	private function UnitDamageReduce takes nothing returns nothing //减免伤害[内部]
        ...
        call SaveReal(HT,GetHandleId(ue),-1,LoadReal(HT,GetHandleId(ue),-1)-rdce)
	     		//set dam = life-dam
     			//if life<dam then
				      call SetUnitState(ue ,UNIT_STATE_LIFE, GetUnitState(ue,UNIT_STATE_MAX_LIFE))
		     	//else
    ...
    private function UnitDamagedAction takes nothing returns nothing
		...
		local integer i = 1
		local weapontype weapon = Xg_DamPlus_GetWTP()
		...
		damplus_dbg call BJDebugMsg("debug[441]: "+R2S(dam)+" = " + R2S(newdam)+" - "+R2S(olddam))
		if dam > 0.0 then //--补足[伤害差>0 !与! 法术伤害]--
			if life - newdam > 0.405 and  olddam>0.0 then //非致死伤害  原版伤害
				damplus_dbg call BJDebugMsg("非致死伤害 来源:"+GetUnitName(us))
				...
				//call EnableTrigger(Xg_DamPlus_Trg)
			elseif olddam < 0.0 and life - newdam > 0.405 then //非致死的法术伤害 修正伤害
				set t = CreateTimer()
				set key = GetHandleId(t)
				call SaveUnitHandle(HT, key, 1, ue)
				if minus < -olddam then // 81/100   20
					set dam = newdam + minus //20+19
				endif
				// 15/20   30>10
				call SaveReal(HT,key,2, dam - 0.001)
				call SaveReal(HT,GetHandleId(ue),-1,LoadReal(HT,GetHandleId(ue),-1)+ReduceValue)
				call DisableTrigger(Xg_DamPlus_Trg)
				call UnitDamageTarget( us,ue, 0.001, false, rng, ATTACK_TYPE_CHAOS, DAMAGE_TYPE_UNIVERSAL, weapon ) //非远程伤害会受到冰甲影响
				call EnableTrigger(Xg_DamPlus_Trg)
				//call SetUnitState(ue, UNIT_STATE_LIFE, life-newdam)
				call TimerStart(t, 0.0, false, function MagicDamageFix)
				set t=null
			elseif olddam==0.0 and life-dam>0.405 then //触发伤害
				call SetUnitState(ue, UNIT_STATE_LIFE, life - dam-0.001 )
				call DisableTrigger(Xg_DamPlus_Trg)
				call UnitDamageTarget( us,ue, 0.001, false, rng, ATTACK_TYPE_CHAOS, DAMAGE_TYPE_UNIVERSAL, weapon ) //非远程伤害会受到冰甲影响
				call EnableTrigger(Xg_DamPlus_Trg)
			elseif life > 0 then
				damplus_dbg call BJDebugMsg("致死伤害 凶手: "+ GetUnitName(us) + "生命值: " + R2S(life) + " 伤害: " + R2S(dam))
				call SetUnitState( ue, UNIT_STATE_LIFE, 500.0 )
				//call UnitRemoveAbility( ue,PhyAbil)
				call DisableTrigger(Xg_DamPlus_Trg)
				//loop //既然已经用到混乱伤害来获取护甲那就放弃枚举攻击类型直接使用混乱了 节约效率
					call UnitDamageTarget(us,ue, 5000.00, false, false, ATTACK_TYPE_CHAOS, DAMAGE_TYPE_UNIVERSAL, weapon )
				//	set i = i + 1
				//	exitwhen i == 7
				//endloop
				call EnableTrigger(Xg_DamPlus_Trg)
		...
		elseif dam == 0.0 then
			...
			if maxlife - life >= dam then
				...
				call SetUnitState(ue, UNIT_STATE_LIFE, life + dam)
				if i <= 0 then
				damplus_dbg call BJDebugMsg("debug[489]: 减伤 "+ R2S(dam)+"  HP "+R2S(life))
					set t = CreateTimer()
					set key = GetHandleId(t)
	...
	function XG_UnitDamTarPlus takes unit us,unit ue,real dam,boolean atk,boolean range,attacktype atp,damagetype dtp,weapontype wtp returns nothing
		...
		if (testd!=0.)and not( phy==false and IsUnitType(ue, UNIT_TYPE_MAGIC_IMMUNE) ) then
		  //call UnitDamagedAction()
		  call TriggerExecute( Xg_DamPlus_Trg )
		endif
	endfunction
	private function AOE_A takes unit us,unit ue,real dam,boolean atk,boolean range,attacktype atp,damagetype dtp,weapontype wtp returns nothing
			local integer id=ExecuteQueId[0]+1
			set ExecuteQueId[id]=1
			//call FlushChildHashtable(ExecuteQue,id)
			call SaveUnitHandle(ExecuteQue,id,HA_UNS, us)
			...
			call SaveBoolean(ExecuteQue,id,HA_TRG,true)     //触发伤害
			call SaveBoolean(ExecuteQue,id,HA_ATK,atk)     //攻击伤害
			call SaveBoolean(	ExecuteQue,id, HA_PHY, LoadBoolean(Const,AOEId,HA_PHY))
			call SaveBoolean(ExecuteQue,id,HA_RNG,range) //远程伤害
			call SaveInteger(ExecuteQue,id,HA_ATP,GetHandleId(atp))
			call SaveInteger(ExecuteQue,id,HA_DTP,GetHandleId(dtp))
			call SaveInteger(ExecuteQue,id,HA_WTP,GetHandleId(wtp))
			call SaveReal(	ExecuteQue, id,HA_Odam,0.0)
			call SaveReal(	ExecuteQue, id, HA_OrigDam, dam)
			call Xg_DamPlus_SetSign(LoadInteger(Const,AOEId,0))
			set ExecuteQueId[0]=id //设置栈索引
			call DamAction( us, ue, dam, atp != ATTACK_TYPE_NORMAL, true, range, atp, dtp, wtp, dtp == DAMAGE_TYPE_UNIVERSAL )
			if (testd!=0.) and not( atp == ATTACK_TYPE_NORMAL and IsUnitType(ue, UNIT_TYPE_MAGIC_IMMUNE) ) then
		  	//call UnitDamagedAction()
			  call TriggerExecute( Xg_DamPlus_Trg )
		endif
	endfunction
	private function AOE_Filter takes nothing returns nothing
		local unit u = GetFilterUnit()
		if IsUnitType(u, UNIT_TYPE_STRUCTURE) == LoadBoolean(Const,AOEId,1) /*建筑*/ \
		and IsUnitType(u, UNIT_TYPE_ANCIENT) == LoadBoolean(Const,AOEId,2)/*古树*/ \
		and IsUnitType(u, UNIT_TYPE_UNDEAD) == LoadBoolean(Const,AOEId,3)/*不死族*/ \
		and IsUnitType(u, UNIT_TYPE_HERO) == LoadBoolean(Const,AOEId,4)/*英雄*/ \
		and IsUnitEnemy(u, Player(LoadInteger(Const,AOEId,-1))) \
		then
			call AOE_A(LoadUnitHandle(Const,AOEId,HA_UNS),u,LoadReal(Const,AOEId,HA_DAM),LoadBoolean(Const,AOEId,HA_ATK),LoadBoolean(Const,AOEId,HA_RNG),ConvertAttackType(LoadInteger(Const,HA_CONS,LoadInteger(Const,AOEId,HA_ATP))),\
							   ConvertDamageType(LoadInteger(Const,HA_CONS,LoadInteger(Const,AOEId,HA_DTP))),\
							   ConvertWeaponType(LoadInteger(Const,HA_CONS,LoadInteger(Const,AOEId,HA_WTP))))
		endif
		set u = null
	endfunction
	function XG_DamPlus_AOE takes unit us,location p,real radius,real dam,boolean atk,boolean range,attacktype atp,damagetype dtp,weapontype wtp,integer sign,boolean build,boolean ancient,boolean undead,boolean hero,boolean unt returns nothing
		local integer id = AOEId + 1
		local group g = CreateGroup()
		local boolean phy = atp != ATTACK_TYPE_NORMAL
		call SaveUnitHandle(Const, id, HA_UNS, us)
		call SaveReal(Const, id, HA_DAM, dam)
		//call SaveBoolean(Const, id, HA_TRG, true) //省略
		call SaveBoolean(Const,id,HA_ATK,atk)     //攻击伤害
		call SaveBoolean(Const,id, HA_PHY, phy)
		call SaveBoolean(Const,id,HA_RNG,range) //远程伤害
		call SaveInteger(Const,id,HA_ATP,GetHandleId(atp))
		call SaveInteger(Const,id,HA_DTP,GetHandleId(dtp))
		call SaveInteger(Const,id,HA_WTP,GetHandleId(wtp))
		//call SaveReal(	Const, id,HA_Odam,0.0)
		call SaveReal(	Const, id, HA_OrigDam, dam)
		call SaveInteger(Const,id,0,sign)
		call SaveInteger(Const,id,-1,GetPlayerId(GetOwningPlayer(us)))
		call SaveBoolean(Const,id,1,build)
		call SaveBoolean(Const,id,2,ancient)
		call SaveBoolean(Const,id,3,undead)
		call SaveBoolean(Const,id,4,hero)
		call SaveBoolean(Const,id,5,unt)
		set AOEId = AOEId + 1
...
// 注册事件 接受普通攻击【1】 接受技能伤害【2】 接受触发伤害【3】 伤害被格挡【4】 伤害处理结束【5】吸血【6】 生命偷取【7】 
	...
	private function gcTrigger takes nothing returns nothing
		local timer t = GetExpiredTimer()
		local integer hTimer = GetHandleId(t)

		call DestroyTrigger( LoadTriggerHandle(HT, hTimer, 1) )
		call DestroyTimer(t)

		call FlushChildHashtable(HT, hTimer)
		set t = null
	endfunction

	private function gcEvent takes nothing returns nothing
		local timer t = CreateTimer()
		local group g = CreateGroup()
		...
		call DisableTrigger( Xg_DamPlus_Trg )
		call SaveTriggerHandle(HT, hTimer, 1, Xg_DamPlus_Trg)
		call TimerStart(t, 15.00, false, function gcTrigger) //延迟GC 实时GC会导致莫名其妙地形不可通行 单位无法通过
	...
	function Xg_DamPlus_ON takes integer abil returns nothing
		...
		call TimerStart( CreateTimer(), 300, true, function gcEvent )
