#ifndef WenHaoD3DIncluded
#define WenHaoD3DIncluded

library WenHaoD3D requires WenHaoJapi
    globals
		...
		private gamecache sync = InitGameCache("WenHao.sync")
    endglobals
    ...
    function DestroyTexture takes integer texture returns nothing
		...
		call FlushChildHashtable(Event,texture)
    endfunction
    ...
    function XG_TextrueRemoveEvent takes trigger trg, integer hwnd returns nothing
		local integer id = LoadInteger(Event, hwnd, GetHandleId(trg))
		local integer able = LoadInteger(Event, hwnd, 1) - 1
		if id != 0 then
			call RemoveSavedHandle(Event, hwnd, -id)

			if able <= 0 then
				call SaveInteger(Event, hwnd, 0, 0 )
				call SaveInteger(Event, hwnd, 1, 0 )
			else
				call SaveInteger(Event, hwnd, 1, able )
			endif
		endif
	endfunction
	...
	private function Event_Redirect takes nothing returns nothing
		local integer hwnd = GetTriggerTexture()
		local integer total = LoadInteger(Event, hwnd, 0)
		local integer i = 1
		local trigger trg
		loop
			exitwhen i > total
			set trg = LoadTriggerHandle(Event, hwnd, -i)
			if trg != null then
	...
	function XG_TextureRegEvent takes trigger trg, integer hwnd returns nothing
		local integer total = LoadInteger(Event, hwnd, 0) + 1
		local integer i = 0
		//正数范围 = 2 - 1000 事件句柄 负数范围 = 触发器列表 |  0 = 触发器最大值 1 = 可用触发器(为0时设置触发器数也为0)
		if LoadBoolean(Event, hwnd, 0) == false then
			loop
				call SaveInteger(Event, hwnd, i+2, TextureAddEvent( hwnd, i, function Event_Redirect, function Event_Redirect ) )
				set i = i + 1
				exitwhen i > 0xFF
			endloop
			set i = 0x200
			loop
				...
				exitwhen i > 0x205
			endloop
			call SaveBoolean(Event, hwnd, 0, true)
		endif
		if LoadInteger(Event, hwnd, GetHandleId(trg)) == 0 then
			call SaveInteger(Event, hwnd, 0, total)
			call SaveTriggerHandle(Event, hwnd, -total, trg)
			call SaveInteger(Event, hwnd, GetHandleId(trg), total)
		endif
...
endlibrary
#include "XueYue\\SystemLibs\\WenHaoJapi\\D3D.j"
#include "XueYue\\SystemLibs\\WenHaoJapi\\LocalJapi.j"
#endif
