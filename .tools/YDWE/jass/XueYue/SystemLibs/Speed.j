...
library XGSpeed requires XGbase
	...
	globals
		...
		private constant integer SPD_OVER = StringHash("speedOverFlow")
	endglobals
	//删除减速
	function Xg_DSpeed takes integer id returns nothing
		...
		local real overSpeed = LoadReal(Xg_htb, hid,SPD_OVER)
		local real speedEx = GetUnitMoveSpeed( u ) - Low + overSpeed
		if speedEx > MaxSpeed then
			call SaveReal(Xg_htb, hid, SPD_OVER, speedEx - MaxSpeed)
		elseif speedEx < MinSpeed then
			call SaveReal(Xg_htb, hid, SPD_OVER, speedEx - MinSpeed)
		else
			call SaveReal(Xg_htb, hid, SPD_OVER, 0)
		endif
		call SetUnitMoveSpeed( u, speedEx)
	...
	function Xg_SpeedA takes unit u,integer ty,real r,real keep,string ui,string text returns integer
		...
		if keep <=0 or GetUnitState(u, UNIT_STATE_LIFE) < 0.405 then
