...
local function precompile(code, output_func, lua_codes)
	...
	local start1, end1 = string_find(code, start_tag, 1, true)
	local start2 = nil
	local end2 = 0
	local cEqual = string_byte('=', 1)
	while start1 ~= nil do
		if start1 > end2 + 1 then
			table_insert(lua_codes, output_func..'('..string_format("%q", string_sub(code, end2 + 1, start1 - 1))..')')
		end
		start2, end2 = string_find(code, end_tag, end1 + 1, true)
		if start2 ~= nil then
			if string_byte(code, end1 + 1) ~= cEqual then
				table_insert(lua_codes, string_sub(code, end1 + 1, start2 - 1))
			else
				table_insert(lua_codes, output_func..'('..string_sub(code, end1 + 2, start2 - 1)..')')
			end
			start1, end1 = string_find(code, start_tag, end2 + 1, true)
		else
			-- end tag not found
			end2 = end1
			start1 = nil
	...
	table_insert(lua_codes, output_func..'('..string_format("%q", string_sub(code, end2 + 1))..')')
	return 
end
...
function template:do_compile(op)
	...
	local lua_codes = {''}
	table.insert(lua_codes, "local table_insert = table.insert; local __jass_result__ = {''}")
	table.insert(lua_codes, "local function __jass_output__(str) table_insert(__jass_result__, str) end")
