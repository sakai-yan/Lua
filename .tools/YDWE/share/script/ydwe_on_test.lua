...
local function host_save_config(curdir, mappath, autostart)
    ...
    local ok, num = pcall(mapdump, mappath, jasspath,
        function(s)
            of:write(s .. '\n')
        end
    )
...
local function host_test(commandline, mappath)
	...
	local num = host_save_config(curdir, mappath, host_test + 1)
	...
	local  localName = reg["userlocal"]

	if not localName or localName == '' or localName == ' ' or localName == '\20' then
		reg["userlocal"] = { registry.REG_SZ, '闆硶' }
	end
	...
	return cmd, num --getplayernum(mappath)
end
...
function event.EVENT_TEST_MAP(event_data)
    ...
    --------------------
    xjass:w2l_slk(mappath:string())
    local mpq = stormlib.open ( mappath )
    local testOn11
    if mpq then
        if mpq:has_file( "TestOn11" ) then
            testOn11 = true
            log.debug( 'Test On 11' )
        end
        mpq:close()
    end
    --------------------
    ...
    if testOn11 then
		xjass:TestOn11(event_data.application_name, commandline, n, mappath )
		log.debug("*********************  test 11 *********************")
		return 0
	end
