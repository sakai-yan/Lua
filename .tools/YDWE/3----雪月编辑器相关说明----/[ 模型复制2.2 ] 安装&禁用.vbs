...
'      插件基础信息定义
'--------------------------
plugin_name = "模型复制 2.2"
' -* 安装说明
plugin_msg_tip = "在编辑器里使用Shift + C复制选中单位/装饰物的模型贴图，可以在文件夹中直接粘贴" + vbcrlf + "由雪歌修复，关闭编辑器时将不会因为该插件而导致崩溃卡住了"
...
plugin_zips.Add( vbs_path & "\plugins\ModelCopy\2.2.zip" )
plugin_zips.Add( war3_path )

' -> 插件是否已安装 检查条件
Function isInstalled()
	isInstalled = fs.FileExists( war3_path & "\ModelCopy.asi" )
End Function
